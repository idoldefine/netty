...
public final class FixedChannelPool<C extends Channel, K extends ChannelPoolKey> extends SimpleChannelPool<C, K> {
    ...
    private void acquire0(K key, final Promise<C> promise) {
        ...
        if (acquiredChannelCount < maxConnections) {
            ...
            // EventLoop
            Promise<C> p = executor.newPromise();
    ...
    private class AcquireListener implements FutureListener<C> {
        private final Promise<C> originalPromise;

        AcquireListener(Promise<C> originalPromise) {
